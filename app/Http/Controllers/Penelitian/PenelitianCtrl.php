...
class PenelitianCtrl extends Controller {
...

    {
        ...
        $users = DB::table('penelitian')
            ->join('dosen', 'dosen.id_dosen', '=', 'penelitian.id_dosen')
            ->select('*');
     ...
     protected function createActionsColumn($data){   
        return '
            <a onclick="detail('.$data->id_penelitian.')" class="btn btn-primary btn-xs"><span class="fa fa-eye"></span></a>
            <a onclick="edit('.$data->id_penelitian.')" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-pencil"></span></a>
